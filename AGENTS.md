...
Verified against `origin/main` at `e9f0a0b` plus the `fix/ultra-ext-2026-09-25` changes (extension `1.0.26`) on 2026-09-25, against the batch D changes (`fix/ultra2-ext`, extension `1.0.27`) on 2026-09-26, and against the round-3 batch 6 changes (`fix/ultra3-ext`, extension `1.0.28`) on 2026-09-26. If this file and the code disagree, the code wins: fix this file in the same PR (section 6).
...
**Gotchas.**
- `cloudIsPro` is still the personal plan only; workspace plans reach scoring through `workspaceRules` alone, so a personal Pro saving to an entitled workspace scores with the workspace's rules (the same rules the app applies in that scope), and their personal rules apply again on Personal.
...
**Purpose and flow.** The History button in the header toolbar (3.17; `aria-expanded` while open) opens "Recent scans", a non-modal `role="dialog"` panel (`aria-label="Recent scans"`: named by its CSS-uppercased header it was announced "RECENT SCANS", KB3-13) run by `use:popover` (3.17): focus lands on the first row (on the empty state's "Scan this page" when there is none), Escape or a press outside closes it, and focus returns to the History button. Rows are the last 10 scans, each left to right: site favicon (`SiteIcon`, 16 px; letter tile when missing or broken), title (or hostname) over hostname, relative time, then the score chip last (`bandClasses(...).chip` from `src/lib/audit/band.ts`, `role="img"` with the "Score N of 100, band" label and the same text as a hover tooltip). Each row button has the tooltip "Open in new tab" (hover and keyboard focus) and an inset focus ring (`FOCUS_RING_INSET`). Empty, the panel shows a clock icon, "No scans yet", "Scan a page and it shows up here, newest first. The last 10 stay on this device." (P-19) and a `BUTTON_PRIMARY` "Scan this page" (I3-13; the `onScan` prop, which `Extension.svelte` sets to `scrape`): it takes focus on open, closes the panel and starts the scan. Rows hover `indigo-500/25` / `dark:white/15` (the app's menu-item strength, 1.38 / 1.55:1). The panel sits `top-full mt-2` under the header's right-hand block, so it clears the toolbar when the header wraps. Clicking an entry opens its URL in a background tab (`active: false`, so a popup stays open). "Clear" is two steps: the first press swaps the button for "Clear all? Yes / No" (11 px, `rose-600` / `dark:rose-300`, focus on Yes); only Yes empties the list; No, Escape (which cancels without closing the panel) or focus leaving the pair cancels. After a clear, focus stays on the panel.
...
- The panel is `.ms-popover` (3.17): opaque in both themes, no `backdrop-blur` (at `white/90` and `popover/95` the landing heading ghosted through it at 320 px). Its separators are `slate-200` / `dark:white/15` (the old `white/40` on white was invisible).
...
**Purpose and flow.** When signed in, every successful scan uploads after its final audit. The dropdown sets where new scans go: "Personal" (the app's word; until 1.0.26 it said "Personal history") or any workspace where the user's role is `owner` or `member`. The list is a `role="menu"` (labelled "Save new scans to") of `role="menuitemradio"` buttons with `aria-checked`; Arrow Up/Down, Home and End move between them and only the checked one is in the Tab order (roving `tabindex`; when the stored workspace is not in the list, nothing is checked and "Personal" takes the Tab stop). The target also picks the scoring rules when it is an entitled workspace (3.8).
...
**Motion.** `src/lib/motion.ts`: `reducedMotion()` reads `matchMedia('(prefers-reduced-motion: reduce)')` (false when `matchMedia` is missing) and `dur(ms)` returns 0 when it is set. Every `fly` / `fade` / `scale` in the shell (Settings drawer and its overlay, History panel, shortcuts help and its overlay, toasts) passes `duration: dur(…)`. The CSS half (P-18) is in `app.css`: under `prefers-reduced-motion: reduce` every element's `transition-duration` and `animation-duration` are 0 (`!important`; the skeleton pulse stands still, colour changes snap) except `.animate-spin`, which slows to 2.4 s and keeps turning like the web app's spinner (P3-08: a spinner stopped after one 0 s turn showed a static arc that read as "stalled"), and movement utilities are `motion-safe:` at the call site - the landing tile's hover lift and press scale, the Retry / Try again / Re-scan press scale, the score ring's sweep, `CharBar`'s width and the skeleton (`motion-safe:animate-pulse`). The three loading spinners (Audit, AI, Site) are plain `animate-spin`. New motion must be `motion-safe:` or take `dur()`.

**Key files.** `src/lib/theme.ts`, `src/lib/motion.ts`, `tailwind.config.js` (`darkMode: 'class'`, the `popover` / `popover-raised` / `field-edge` colours of 3.17). `app.css` sets `color-scheme: light` on `:root` and `dark` on `:root.dark` (R3-24 / P3-02), so number spinners and checkboxes follow the theme class, not the OS.
...
**Turning them off (WCAG 2.1.4).** Settings > Preferences > "Single-key shortcuts" (storage key `shortcuts`, `src/lib/storage/shortcuts.ts`, default on). Off, only `?` (help) and `Esc` work; `/`, `r` and `1`-`6` do nothing. `shortcutFor(key, shiftKey, enabled)` in `keyboard.ts` is the pure mapping. The help sheet shows an amber note when they are off, suffixes the single-key rows "(off)", and always ends with "Single-key shortcuts can be turned off in Settings > Preferences."
...
- Header, left to right: logo (wordmark visible from 400 px); the account control (3.13); one `role="toolbar"` group styled by `src/lib/components/toolbar.ts` (`toolbarButtonClass(active)`, `TOOLBAR_GROUP`; the file also exports the shell's focus rings, below) holding History, Settings, theme toggle and shortcuts `?`. One line at 100 % text from 320 px; the header, its right-hand block (`ml-auto`) and `TOOLBAR_GROUP` all `flex-wrap`, so at 200 % text the block drops under the logo and the toolbar wraps inside itself, and every control stays on screen at 320 and 400 px (R2-16; nothing scrolls sideways, so a control pushed past the edge was unreachable). Hover and "on" differ (V2-08): idle hover is a neutral `slate-900/5` wash with darker ink, `toolbarButtonClass(true)` is a brand tint (`indigo-600/10`, `indigo-700` ink; `indigo-400/20` / `indigo-200` dark) for an open menu or sheet AND for the theme toggle while dark is on (`aria-pressed`). The theme toggle has ONE state model: a fixed name "Dark theme" with `aria-pressed` true in dark mode (the old name flipped with the state AND carried `aria-pressed`). Every button has a `use:tooltip` equal to its `aria-label`; the account control's tooltip is "Sign in to sync scans to your account" or "Signed in as <email> - saving scans to <target>". Arrow Left/Right, Home and End move focus inside the toolbar. The surface toggle lives in Settings (3.2).
...
- Views: `landing` (heading + value line, then Grid: the "Scan this page" card is a `<button type="button">` with the brand fill - white on `indigo-600`, hover `indigo-700` - the view's one primary action (T-07 / V2-10), first in the tab order after the header, Enter / Space start the scan), `loading` (`Skeleton`), `error` (`ErrorState`: `h2` "Couldn't scan this page" with `tabindex="-1"` that takes focus on mount - the button that started the scan is gone - then the plain `reason`, Retry, a `<details>` "Details" with the raw `detail` when there is one, links to docs and GitHub issues), `empty` (`EmptyState`: "No meta tags found", Try again, docs link), `results` (`Tabs` with Tags, Previews, Audit, Site, AI, Compare).
- `Screen`: glass card wrapper. `Grid`: landing action cards (`GridProps` in `Grid.ts`). `<main>` scrolls (`overflow-y-auto`, `-m-1 p-1` so a focus ring is not clipped) in every view but `results`, which keeps its own scroll area between the tabs and Re-scan (R3-10: at 200 % text in a 320 px panel the wrapped header pushed "Scan this page" below the fold of the `h-screen overflow-hidden` root and a pointer could not reach it).
- Text sizes are rem (R3-20): Tailwind's `text-xs` and up, and `text-[0.6875rem]` (11 px at 100 %) as the floor for small labels, chips, hints and footers. Never `text-[Npx]`: px text ignored the user's text size (at 200 % descriptions stayed 11 px under 24-32 px titles). The one px font size left is `SiteIcon`'s letter tile, sized with its px box.
- Toasts (R2-27, same meaning as the web app's): `toast(message, variant)`; at most 3 visible, bottom-right (`left-4 right-4 items-end`). Each stays `toastDuration(message, variant)` = 3.5 s + 45 ms per character (the app's, V3-12; it was 2 s), capped at 9 s; errors at least 6 s (`toast.ts`, specced in `toast.spec.ts`). Every toast has a 24 px "Dismiss notification" x (`dismissToast(id)`); its hover is `white/15` on the coloured and dark fills, `slate-900/10` only on the light dark-mode note. Fills are the app's exact values, in both themes: success white on `#007a55`, error white on `#c70036` (the app's Tailwind 4 emerald-700 / rose-700, written as values because Tailwind 3's steps differ), default white on `slate-900` / dark `slate-900` on `slate-50`. Two always-mounted live regions, like the app (KB3-12): notes in `role="status" aria-live="polite"` above, errors in `role="alert" aria-live="assertive"` below; a toast is text added to an existing region, never a freshly inserted `role="alert"` node. (Until 1.0.26 every toast lived 1.5 s with no close, and dark success / error were the popover tint at 1.10:1 vs the page.)
...
- Floating surfaces (R3-24, the web app's `ms-popover` / DESIGN.md "Floating surfaces"): the two header menus (History, account popover) are `.ms-popover` (`app.css`): light opaque white, `slate-300` edge, `slate-900/15` shadow; dark `popover-raised` (`#3a3178`, `tailwind.config.js`, the app's `--color-popover-raised`), `white/35` edge, `black/50` shadow. Measured at 400 px over the landing card: light surface 1.17 / edge 1.27:1 vs the card (app 1.10 / 1.35); dark surface 1.39 / edge 3.83:1 (app 1.39 / 3.82). The old `white/40` / `white/10` edges were 1.03 / 1.02. Inside `.ms-popover` in dark, `.ms-muted` steps up to slate-300 (slate-400 on `popover-raised` is 4.22:1; 7.48 now). Items are `.ms-menu-item`: hover `indigo-500/25` / `dark:white/15` (1.38 / 1.55:1 vs the list, the app's numbers), `aria-checked` adds `indigo-500/10` / `white/10` and brand ink (1.35:1 dark); hover and press still paint over the picked row. Settings' sync dot is `emerald-600` / `dark:emerald-400` too (C3-15), and an invalid Settings field takes a `rose-600` / `dark:rose-300` keyboard ring. The `popover` colour (`#2a2159`, the app's `--color-popover-dark`) stays the dark background of the Settings drawer and help (`dark:bg-popover/95` on a dimmed, blurred overlay). Toasts carry the web app's solid fills (above). No `backdrop-blur` on the header menus: at `white/90` and `popover/95` the landing heading showed through them at 320 px. Never `slate-900` for a floating surface.
- Focus rings: `FOCUS_RING` (`toolbar.ts`) is `focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-600 focus-visible:ring-offset-2 focus-visible:ring-offset-transparent dark:focus-visible:ring-indigo-300`, on every button and link in the shell (toolbar, account control and its popover, History Clear / confirm, help Close, every drawer button and link including the footer links, the Audit "what changed" button, `BUTTON_SECONDARY`, the pin / copy card buttons and the export pills); `FOCUS_RING_INSET` is the same ring drawn inside for the full-bleed History rows. `FOCUS_RING_ON_FILL` is the same ring with a page-coloured offset (`ring-offset-white` / `dark:ring-offset-slate-950`, the app's brand-fill rule) for every brand or danger fill: "Scan this page", Re-scan, `BUTTON_PRIMARY` (Retry, Try again, Compare, Sign in, Save, Go Pro, History's "Scan this page"), `BUTTON_DANGER` and the category chips (R3-22: with a transparent offset the indigo-600 ring touched the indigo-600 fill, 1.00:1, and read as a 4 px bigger button). Never put both on one element. The old `ring-indigo-500/40` blended to about 1.7:1; the solid ring measures about 6.3:1 on white and 7.2:1 / 9.0:1 on `popover` / `slate-900` (WCAG 2.4.13 needs 3:1). Fields (`.ms-input`) keep a mouse-focus border + soft `indigo-400/30` ring and take the shared ring on keyboard focus. The Surface radio labels use the same colours with `focus-within:`. **Forced colors (R-26):** rings are box-shadows, which Windows contrast themes drop. Tailwind 3's `focus:outline-none` is `outline: 2px solid transparent`, which forced colors repaints, and `app.css` makes it explicit: under `@media (forced-colors: active)` every `:focus-visible` gets `outline: 2px solid Highlight` (offset 2 px), and a label whose `sr-only` input is focused (the Surface radios) gets the same. Never write a bare `outline: none`.
- Text fields: one look, `.ms-input` (`app.css`; the web app's `ms-input`, R3-24): `rounded-xl`, `px-3 py-2 text-sm` (38 px), light `indigo-100/80` fill with the `field-edge` border (`#737e97`), dark an inset `slate-950/60` well with `field-edge-dark` (`#7880b5`), muted placeholder. Used by the sign-in fields, the tag search (`pl-9` for its icon), the Compare URL field (`aria-label="URL to compare"`) and the Settings number inputs (invalid: `rose-500` / `dark:rose-400` edge). The old three shapes (`rounded-lg`, two `rounded-full` pills) and the lighter `white/5` dark slab are gone. Measured on the Screen (Compare field), light: fill 1.08, edge 3.36 vs fill, 3.62 vs the card, placeholder 6.25 (app field on a card 1.12 / 3.33 / 3.72, placeholder 6.22 on the page); dark 1.14 / 4.80 / 4.19, placeholder 7.07 (app row card 1.16 / 4.99 / 4.28). In the dark sign-in popover the edge is 4.48:1 vs its fill but 2.94:1 vs `popover-raised`.
- Muted text (R-33): secondary text (captions, section labels, hints, idle icons) uses `.ms-muted` (`app.css`, components layer so a hover utility still wins) = the `muted` colour in `tailwind.config.js`: `#475569` (slate-600) light, `#94a3b8` (slate-400) dark - 6.71:1 on the light Screen, 7.58:1 on white, 5.61:1 on `popover`. Never a raw `text-slate-400` / `text-slate-500` (they measured 2.27:1 and 3.02:1); `grep -rnE "text-slate-(400|500)" src` finds only `Previews/DiscordPreview.svelte`'s host caption, which sits on the fixed dark Discord mock (`bg-[#2b2d31]`) in both themes and keeps `text-slate-400` (5.4:1 there; `.ms-muted` light would be 1.8:1). Fixed-colour preview mocks are exempt from the token.
...
- Buttons (K-18): `src/lib/components/button.ts` exports `BUTTON_PRIMARY` (white on `indigo-600`, hover `indigo-700` - white 12 px on the old `indigo-500` hover was 4.47:1), `BUTTON_SECONDARY` (neutral, `slate-500` / `slate-400` edge) and `BUTTON_DANGER` (white on `rose-600`, hover `rose-700`; `rose-500` was 3.67:1). One shape: `rounded-xl`, 12 px semibold, `min-h-[2.25rem]` (grows with the text); primary and danger take `FOCUS_RING_ON_FILL`, secondary `FOCUS_RING`. Used by the header Sign in (secondary), the sign-in popover, Settings Save / Reset / "Yes, reset" / Go Pro, the Empty / Error states' retry, History's empty-state "Scan this page" and the Compare submit (K3-03 / C3-10: it was a hand-rolled pill hovering to `indigo-500`, 4.47:1, with no focus ring; hover is now `indigo-700`, 7.90:1). Use one primary per view. Icon buttons on cards and dialogs (pin, copy, sign-in Close, shortcuts Close) are 32 px like the toolbar and Settings close (K3-15).
- Scores: `src/lib/audit/band.ts` (`bandFor`, `bandLabel`, `scoreLabel`, `bandClasses`) is the only place a score turns into a band, a label or colour classes; the History chip, the Audit ring, the Compare numbers and the cloud payload's `band` all use it, and so do the result views' status marks (R2-29): Audit rule icons (`stroke` via `bandForVerdict(status)`), `CharBar` (`fill`: over max -> fail, under min -> warn), AI-check dots and the AI-ready chip (`fill` / `chip`; `info` dots are `slate-500` / `slate-400`), the pinned star (`warn.stroke`, amber-700) and the sitemap notes (`ink`: `amber-800` / `rose-700` text, 12 px). Measured on the light result card: rule icons and bars emerald-600 3.61, amber-700 4.84, rose-600 4.50; notes amber-800 6.84 (the old `-500` marks were 1.90-2.25, the amber-600 note 2.87). `BandClasses` = `text`, `chip`, `stroke`, `fill`, `ink`. Light hues are `-600 / -700` like the web app (ring stroke and Compare number: emerald-600 3.34:1, amber-700 4.45:1, rose-600 4.16:1 on the light Screen; the old `-500` measured 2.25 / 1.90 / 3.25); dark keeps `-400`. Chips are the web app's `ms-badge-*` (R3-23): the band's fill at 15 % (`bg-emerald-600/[.15]`, `bg-amber-700/[.15]`, `bg-rose-600/[.15]`; dark `-400/[.15]`) with `-800` text (`amber-900` for warn: `-800` read 4.30 on a hovered History row) and `-300` in dark; `CHIP_NEUTRAL` (`slate-500/10`, `slate-600` text) is the app's `ms-badge-neutral`. Used by the History score chip, the AI-ready chip, the Site cards' Found / Missing / Not present and Settings' "Unsaved changes". Measured light: History 6.45 / 5.76 / 6.32 at rest (were 4.09 / 3.88 / 4.33), Site cards 6.22 / 6.09 / 6.46; dark 5.62 / 5.78 / 4.80. Placement rule shared with the web app: identity left, score last on the right. Tailwind 3 gotcha: `bg-*/15` compiles to nothing (no 15 in the opacity scale) - write `/[.15]` (or use `/20`).
...
- Same words as the web app and the site: workspace / Personal, "Last scan", "Open what changed".
