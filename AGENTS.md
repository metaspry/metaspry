...
- Covered by unit tests: `actions/tooltip.ts` (options, `aria-describedby` merge, listener wiring; node env with a fake element), `audit/rules.ts`, `audit/asyncRules.ts`, `audit/url-match.ts`,
