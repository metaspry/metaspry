...
**Motion.** `src/lib/motion.ts`: `reducedMotion()` reads `matchMedia('(prefers-reduced-motion: reduce)')` (false when `matchMedia` is missing) and `dur(ms)` returns 0 when it is set. Every `fly` / `fade` / `scale` in the shell (Settings drawer and its overlay, History panel, shortcuts help and its overlay, toasts) passes `duration: dur(…)`. The CSS half (P-18) is in `app.css`: under `prefers-reduced-motion: reduce` every element's `transition-duration` and `animation-duration` are 0 (`!important`; spinners and the skeleton pulse stand still, colour changes snap), and movement utilities are `motion-safe:` at the call site - the landing tile's hover lift and press scale, the Retry / Try again / Re-scan press scale, the score ring's sweep, `CharBar`'s width, the three spinners (`motion-safe:animate-spin`) and the skeleton (`motion-safe:animate-pulse`). New motion must be `motion-safe:` or take `dur()`.
...
- Toasts (R2-27, same meaning as the web app's): `toast(message, variant)`; at most 3 visible, bottom-right (`left-4 right-4 items-end`). Each stays `toastDuration(message, variant)` = 2 s + 45 ms per character, capped at 9 s; errors at least 6 s (`toast.ts`, specced in `toast.spec.ts`). Every toast has a 24 px "Dismiss notification" x (`dismissToast(id)`). Fills are the app's, in both themes: success white on `emerald-700` (5.48:1), error white on `rose-700` (6.29:1), default white on `slate-900` / dark `slate-900` on `slate-50`. Errors are `role="alert"`; the rest render inside one persistent `aria-live="polite"` region. (Until 1.0.26 every toast lived 1.5 s with no close, and dark success / error were the popover tint at 1.10:1 vs the page.)
