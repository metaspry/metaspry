 ...
 *
 * A port of the web app's `validateAuditSettings` (`app/src/lib/firebase/auditSettings.ts`), so
 * both surfaces reject the same input with the same words; the two repos cannot share code. The
 * old drawer dropped bad values silently, which left a cleared box looking empty while the previous
 * value still scored, and accepted `min > max`. Pure, so the tests cover it without a DOM.
 */
...
export function validateSettings(s: Settings): SettingsProblem[] {
  ...
  if (WEIGHT_KEYS.some((k) => bad(w[k]))) {
    problems.push({ field: 'weights', message: 'Weights must be 0 or more.' });
